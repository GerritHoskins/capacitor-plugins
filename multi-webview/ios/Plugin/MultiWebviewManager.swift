...
class MultiWebviewManager: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    func createWebview(
        ...
        if webviews[id] != nil {
            throw MultiWebviewError.webviewAlreadyExists
        }

        guard let viewController = plugin?.bridge?.viewController else {
            throw MultiWebviewError.noViewController
        }
    ...
    func loadUrl(id: String, urlString: String) throws {
        ...
        guard let url = URL(string: urlString) else {
            throw MultiWebviewError.invalidURL
        }
    ...
    func executeJavaScript(id: String, code: String, completion: @escaping (Any?, Error?) -> Void) {
        guard let container = webviews[id] else {
            completion(nil, MultiWebviewError.webviewNotFound)
            return
        }
    ...
    func sendMessage(id: String, data: Any) throws {
        ...
        guard let jsonString = String(data: jsonData, encoding: .utf8) else {
            throw MultiWebviewError.webviewNotFound // Use a better error
        }
